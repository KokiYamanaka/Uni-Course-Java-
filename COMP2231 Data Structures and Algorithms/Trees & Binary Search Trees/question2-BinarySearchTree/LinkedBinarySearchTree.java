...
// LinkedBinarySearchTree.java - This program class represents binary search tree.
// It includes methods such as removing max/min of tree, getting left/right subtree, and balance a tree.
////*****************************************************************************************************************************************************************************************
...
public class LinkedBinarySearchTree<T> extends LinkedBinaryTree<T> implements BinarySearchTreeADT<T> {
    ...
    public T removeMax() throws EmptyCollectionException {
        ...
        // if tree is empty, then throw an exception
        if (isEmpty())
            throw new EmptyCollectionException("LinkedBinarySearchTree");
        // otherwise find the farthest maximum node 
        else {
            // if right rood child don't exist, then root is max
            if (root.right == null) {
                result = root.element;
                root = root.left;
            }
            // else loop through to find right most node
            else {
                BinaryTreeNode<T> parent = root;
            ...
            // decrement mod count since inner data struture is changed
            modCount--;
        }
    ...
    public T findMin() throws EmptyCollectionException {
        // set temporary variable to store result
        T result = null;

        // if tree is empty, then throw an exception
        if (isEmpty())
            throw new EmptyCollectionException("LinkedBinarySearchTree");
        // else find minimum which is the farthest left node of tree 
        else {
            // set current pointer to root
            BinaryTreeNode<T> current = root;

            // iterate to find farthest left node
            while (current.left != null)
                current = current.left;

            // get the current's element
            result = current.element;
        }
    ...
    public T findMax() throws EmptyCollectionException {
        ...
        if (isEmpty())
            throw new EmptyCollectionException("LinkedBinarySearchtree");
        // else find maximum node 
        else {
            BinaryTreeNode<T> current = root;

            // loop through to find farthest right node
            while (current.right != null)
                current = current.right;
    ...
    // balance binary search tree by creating an array from an inorder traversal.
    public void balanceTree() {
        // size of the binary search tree
        int treeSize = size();

        // create array to store tree values
        T[] inOrderTreeArray = (T[]) new Object[treeSize];
        ...
        // fill array with tree values
        while (iter.hasNext()) {
            inOrderTreeArray[i] = iter.next();
            i++;
        }

        // balance tree
        root = buildBalancedTree(inOrderTreeArray, 0, treeSize - 1);
    }
    
    // method to balance the tree.
    // accepts an array that represents sorted binary search tree.
    private BinaryTreeNode<T> buildBalancedTree(T[] inOrderTreeArray, int min, int max) {
        // recursion initial condition
        if (min > max)
            return null;

        // midpoint of current array
        int midpoint = (min + max) / 2;

        // build balanced tree recursively
        BinaryTreeNode<T> node = new BinaryTreeNode<T>(inOrderTreeArray[midpoint]);

        // left side of balanced tree
        node.setLeft(buildBalancedTree(inOrderTreeArray, min, midpoint - 1));

        // right side of balanced tree
        node.setRight(buildBalancedTree(inOrderTreeArray, midpoint + 1, max));
